...
const addToDeliveredDB = async () =>{
    // console.log(data.orderData)
  await axios({
    method:'post',
    url:`${process.env.REACT_APP_BACKEND_BASE_URL}/deliveredProduct`,
    data:data.orderData
  })
  .then((response)=>{
...
const removeFromPackedDB = async () =>{
await axios.delete(`${process.env.REACT_APP_BACKEND_BASE_URL}/productPackage/${data._id}`)
    .then((response)=>{
    ...
    return(
        ...
        <CCard className="col-lg-8 col-md-8 col-12">
            <CCardHeader className="d-flex justify-content-lg-between align-items-center">
              <span>Packed Order Details</span>
              <CButton type="submit" size="sm" color="primary" onClick={packedHandler}><CIcon name="cil-scrubber"/>Delivered?</CButton> 
            </CCardHeader>
            <CCardBody>
              <CForm>
...
export default EditPackedProduct